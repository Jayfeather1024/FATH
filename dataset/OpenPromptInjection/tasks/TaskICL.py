...
from .utils import process_gigaword, process_hsol, process_jfleg, process_mrpc, process_rte, process_sms_spam, process_sst2, process_alpaca, process_probes, process_repeat
...
from .alpaca import get_alpaca
from .probes import get_probes
from .repeat import get_repeat
...
class TaskICL:
    ...
    def __init__(self, config, num, icl_num=20):
        ...
        self.preprocess_func = {
            ...
            'jfleg':process_jfleg,
            'alpaca':process_alpaca,
            'probes':process_probes,
            'repeat':process_repeat
        }[self.dataset]
        ...
        self.split = config["dataset_info"]["icl_split"]
        self.num = num
        self.__prepare_data()
    ...
    def get_data_saving_path(self):
        saving_path = f"./data/{self.task}_{self.dataset}_{self.split}_{self.num}_icl"
    ...
    def __split_dataset_and_save(self, raw_data):
        if self.dataset=="alpaca" or self.dataset=="probes" or self.dataset=="repeat":
